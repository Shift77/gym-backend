        ...
        if sub_plans_data:
            ...
            for sub_plan_d in sub_plans_data:
                ...
                if sub_plan_d.get('id', None) in existing_sub_plans_id:
                    sub_plan_instance = models.SubPlan.objects.get(
                        id=sub_plan_d.get('id')
                        )
                    sub_plan_instance.name = sub_plan_d.get(
                        'name',
                        sub_plan_instance.name
                        )
                    sub_plan_instance.week_number = sub_plan_d.get(
                        'week_number',
                        sub_plan_instance.week_number
                        )
                    sub_plan_instance.day_number = sub_plan_d.get(
                        'day_number',
                        sub_plan_instance.day_number
                        )
                    sub_plan_instance.description = sub_plan_d.get(
                        'description',
                        sub_plan_instance.description
                        )

                    if exercises_id_list:
                        valid_id_list = [id for id in exercises_id_list if
                                         models.Exercise.objects.filter(id=id).exists()]  # noqa
